...
(function () {
  ...
  function finishSlapRace() {
    ...
    if (!loser) loser = slapOrder[slapOrder.length - 1];
  ...
  drawPileEl.addEventListener('click', function () {
    ...
    if (hasLegalCard()) return;
    catchSelfIfDrawingUncalled('player');
    reshuffleIfNeeded();
  ...
  function setMode(newMode) {
    ...
    if (unoStartEl.hidden) returnToStartScreen();
  }
